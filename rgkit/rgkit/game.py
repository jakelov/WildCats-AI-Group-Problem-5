...
class Player(object):
    def __init__(self, file_name=None, robot=None, code=None, name=None):
        ...
        :param file_name: -- path to file containing a robot
        :param robot: -- instance of a robot
        :param code: -- source code containing a robot
        :param name: -- argument can be used to set robot's name
        """
        ...
        if file_name is not None:
            try:
                ...
                self._robot_class = Player.get_robot_class(f'{module_name}.Robot')
    ...
    @staticmethod
    def get_robot_class(robot_path):
        """
        RMP: helper for getting the robot in a way that allows debugging.

        :param robot_path: module.Robot
        :return: the robot
        """
        # https://stackoverflow.com/a/19393328
        import importlib
        module_name, func_name = robot_path.rsplit('.', 1)
    ...
    def _get_response(self, game_info, robot, seed):
        ...
        try:
            ...
            self._robot.state = robot.state
            self._robot.action = robot.action
            action = self._robot.act(game_info)
            robot.state = self._robot.state
            robot.action = self._robot.action
    ...
    def _calculate_actions_on_turn(delta, actions):
        ...
        for delta_info in delta:
            ...
            actions_on_turn[loc] = {
                ...
                'damage_caused': delta_info.damage_caused
            }
