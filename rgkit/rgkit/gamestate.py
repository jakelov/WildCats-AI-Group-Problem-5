...
class GameState(object):
    ...
    def add_robot(self, loc, player_id, damage_caused=0, state=None, action=None, hp=None, robot_id=None):
        ...
        if hp > 0:
            ...
            self.zombies[robot['robot_id']] = robot
    ...
    def apply_delta(self, delta):
        ...
        for delta_info in delta:
            # RMP: add all robots living and dead
            # RMP: add_robot keeps track of zombies too
    ...
    def get_game_info(self, player_id):
        ...
        # RMP: add zombies to game state
        game_info.zombies = dict((zombie.robot_id, AttrDict(zombie)) for zombie in self.zombies.values())
        for robot in game_info.zombies.values():
            if robot.player_id != player_id:
                del robot.robot_id
